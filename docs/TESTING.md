...
### Landing row 28 — deploy and registration for futuremagic.de

- Scope: `deploy-sync.sh` (build at `/Orion/`, patch `.htaccess`, diff-sync over FTP, register), `scripts/futuremagic-registry.py` (the registration step), the app's manifest and screenshot, and the pins around them. Ledger row 45.
- Gate: `bash scripts/gate.sh` now runs a **`check:deploy`** step — `python3 scripts/futuremagic-registry.py --selftest` — beside the wasm drift check, plus **313 unit tests** (4 of them new, in `src/domain/deploy/futuremagic.test.ts`) and **41 browser tests**.
- Derived from the live system, not from guesswork: the registry lives at `/webseiten/apps.json` (confirmed by `SIZE` over FTP and by fetching it over HTTP — same 3211 bytes), its entries are `{slug,title,path,updatedAt,manifesto}`, existing entries keep their position while new ones append, and the site resolves a **relative** `screenshot` against the app's own path.
- Pins: the registry upsert (add appends, update keeps position, others untouched, malformed registries refused) and — the interesting one — **byte-identical reproduction of the Windows helper's output**, checked by round-tripping the live registry file. That is what keeps a registration from here to a one-entry diff rather than a whole-file reformat of a file eleven apps depend on. Plus the deploy contract: `application/wasm`, both cross-origin-isolation headers, and the registered path matching the deployed one.
- A real screenshot ships with the app (`public/shot.png`, 962x642), generated by `scripts/make-screenshot.mjs` from the running app. Generating it takes minutes on this box because software Vulkan draws all 614k pixels, so the committed PNG is the artefact and the script is for regeneration.
- **Blocker, with evidence:** the FTP data channel from this box stalls. The control channel is fine (login, `CWD`, `SIZE`, `DELE` all answer), the credentials and paths are right (a `RETR` of `apps.json` succeeded once, 3211 bytes in 1.4s), and the passive data connection is _established_ — then `STOR`, `LIST` and `MKD` receive nothing and time out, in both `curl` (EPSV) and `ftplib` (PASV). So the upload could not be performed from here. The same stall would affect the sibling projects' Linux deploys, which use the same `ftplib` path with a 300-second timeout. The zero-byte files those failed uploads left in `/webseiten/` were deleted again over the control channel, so the webroot is exactly as it was found. `deploy-sync.sh` now reads one small file before building and aborts with that diagnosis (exit 3, nothing uploaded) instead of hanging in a per-file timeout.
